...
def main():
    ...
    # Test nay chi kiem tra binding UI + STOP. Acquisition/Demo da co flow
    # test rieng; dung HR on dinh de khong tao multiprocessing MP36 o day.
    view.waveform.start = lambda: setattr(view.waveform, "cur_bpm", (zone.thr_low + zone.thr_high) / 2)
    view.waveform.stop = lambda: None
    view._confirm_stop = lambda: True
    ...
    def _tick():
        ...
        # Summary nay la mot page rieng (khong con QMessageBox blocking).
        if state["stop_requested"] and view.pages.currentWidget() is view.summary_page:
            view.btn_summary_home.click()
            return
        ...
        if not state["stop_requested"] and _ready_to_stop():
            ...
            print("[2] Bam STOP...")
            QTimer.singleShot(0, _stop_session)
            return
    ...
    def _stop_session():
        view.btn_stop.click()
        print("    STOP handler da tra ve", flush=True)
